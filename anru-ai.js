...
const ADMIN_BOT_NAME = "RUKMANI 😘"; // 👈 Admin ke liye bot ka special naam yahan likho
const NORMAL_BOT_NAME = "AnRu AI Tutor"; // 👈 Baki students ke liye normal naam

let chatHistory = JSON.parse(localStorage.getItem('anru_ai_history')) || [];
let currentImgBase64 = null;

// 🛡️ Admin Check Function: Ye check karega ki login user Admin hai ya nahi
function isAdminUser() {
    try {
        const session = JSON.parse(localStorage.getItem('mceo_sess'));
        return session && session.email.toLowerCase() === ADMIN_EMAIL.toLowerCase();
    } catch(e) { return false; }
}

// --- 3. PREMIUM FULL-SCREEN UI & ADVANCED ANIMATIONS ---
const aiTemplate = `
<style>
...
<div class="ai-modal" id="anruAiModal">
  <div class="ai-chat-box">
    ...
    <div class="ai-header">
      <div class="ai-header-title" id="aiHeaderTitle"><span style="font-size:24px;">🤖</span> AnRu AI Tutor</div>
...
function initAIBot() {
    ...
    // 👑 DYNAMIC HEADER: Admin hai to naam change kardo
    if(isAdminUser()) {
        const headerTitle = document.getElementById('aiHeaderTitle');
        if(headerTitle) headerTitle.innerHTML = `<span style="font-size:24px;">✨</span> ${ADMIN_BOT_NAME}`;
    }
