/* ████████████████████████████████████████████████████████████
      ANRU AI - FINAL PRODUCTION BUILD (ADVANCED UI + FIXES)
████████████████████████████████████████████████████████████ */
...
const aiTemplate = `
<style>
  /* AI Modal - TRUE FULL SCREEN WITH KEYBOARD FIX */
  .ai-modal { display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: #0b091a; z-index: 9999999; flex-direction: column; overflow: hidden; transition: height 0.3s ease; }
  .ai-modal.open { display: flex; animation: slideInRight 0.4s cubic-bezier(0.25, 0.8, 0.25, 1) forwards; }
  ...
  /* Premium Header */
  .ai-header { padding: 16px 20px; background: rgba(11, 9, 26, 0.8); backdrop-filter: blur(20px); border-bottom: 1px solid rgba(255,255,255,0.08); display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 20px rgba(0,0,0,0.4); z-index: 10; }
  ...
  /* Chat Bubbles with Pop Animation */
  .ai-msg { max-width: 85%; padding: 14px 18px; border-radius: 18px; font-size: 14.5px; line-height: 1.5; word-wrap: break-word; animation: bubblePop 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; opacity: 0; }
  ...
  /* Image Preview Box */
  .ai-img-preview { display: none; padding: 12px 20px; background: rgba(0,0,0,0.6); border-top: 1px solid rgba(255,255,255,0.08); position: relative; animation: slideUp 0.3s ease; }
  .ai-img-preview img { height: 70px; border-radius: 10px; border: 2px solid #667eea; object-fit: cover; }
  ...
  /* Footer & Input */
  .ai-footer { padding: 12px 16px; background: rgba(11, 9, 26, 0.95); backdrop-filter: blur(10px); display: flex; gap: 10px; border-top: 1px solid rgba(255,255,255,0.08); align-items: center; padding-bottom: max(16px, env(safe-area-inset-bottom)); z-index: 10; }
...
function initAIBot() {
    ...
    // 🚀 SMART KEYBOARD & VIEWPORT FIX FOR MOBILE
...
window.toggleAIBot = function() {
    ...
    if (modal) {
        ...
        if(modal.classList.contains('open')) {
            ...
            scrollToBottom();
            setTimeout(() => document.getElementById('aiInputBox').focus(), 400);
        }
    }
};
...
window.sendAUMessage = async function() {
    ...
    // Save to LocalStorage (Only keep last 30 messages to avoid crash)
...
// 🧠 FINAL GOOGLE GEMINI API (TEXT + VISION)
async function fetchGeminiResponse(userMessage, imgBase64) {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key=${GEMINI_API_KEY}`;
    
    // Strict Instructions for formatting[span_2](start_span)[span_2](end_span)
